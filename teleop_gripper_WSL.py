#!/usr/bin/env python3
"""
teleop_gripper.py — [WSL VERSION] Browser-previewed hand-pinch teleop for DAS Gripper.
Streams annotated video to http://localhost:8080 and handles hotkeys in the terminal.

This is the WSL version: because WSL usually has no display/X server attached
to the webcam pipeline, video is streamed over HTTP to a browser on the
Windows host and hotkeys are read from the WSL terminal instead of a native
OpenCV window. If you're running on native Linux (real display + webcam,
no WSL passthrough quirks), use teleop_gripper_native.py instead — same
tracking/control logic, but it shows the feed in a real OpenCV window and
reads keys directly, no HTTP server or terminal raw-mode needed.
...
Modes:
    ...
    dual gripper (NEW): plug in both gripper controllers and drive them
    with both hands at once — your left hand drives the left gripper,
    your right hand drives the right gripper.
        python teleop_gripper.py both
    ...
    def do_GET(self):
        if self.path in ("/", "/index.html"):
            ...
            <body style="background:#111; color:#eee; font-family:sans-serif; text-align:center;">
                <h2>DAS Gripper Controller — Hand Teleop</h2>
...
def terminal_input_thread_dual(state_left: HandTeleopState, state_right: HandTeleopState):
    ...
    hand(s) are currently visible — if only one hand is in frame when you
    press 'n'/'f', only that hand's calibration updates."""
    fd = sys.stdin.fileno()
    old_settings = termios.tcgetattr(fd)
    try:
        tty.setcbreak(fd)
...
def _update_ratio_from_landmarks(frame, pts, state: HandTeleopState):
    ...
    state.smoothed_ratio. Identical to what the single-hand loop already
    did inline."""
...
def run_single(args):
    """Single-gripper hand-pinch teleop. Unchanged from the original
    ...
    try:
        while state.running:
            ...
            enc_str = (
                f"{encoder:.4f} m"
                if encoder is not None and enc_age <= 1.0
                else (
                    f"{encoder:.4f} m (stale)"
            ...
            overlay = [
                f"Target: {target:.4f} m   Encoder: {enc_str}",
...
def run_dual(args):
    """Dual-gripper hand-pinch teleop: your left hand drives the left
    gripper, your right hand drives the right gripper, both from the same
    ...
    print("=" * 60)
    print("DUAL-GRIPPER STREAM READY! Open this URL in Chrome/Edge on Windows:")
    print(f"  👉 http://localhost:{args.http_port}")
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Webcam hand-pinch teleop (WSL version)")
